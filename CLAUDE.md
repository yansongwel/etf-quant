...
│   ├── signals.py     # V3.5.1 信号引擎 (IC校准权重、反转因子、vol仓位管理)
...
6. **V4.1 信号引擎** — 18 因子 IC 驱动权重；ma_dev_20d(IC=0.193)最强；买入阈值8分/卖出-8分（V3.5 是12/-15）；ATR移动止盈+RSI背离+放量见顶+趋势内反转+日历效应 5 个新卖出触发器；买入准确率59%/卖出48%（V3.5 买入54%/几乎无卖出）；generate_signal 使用 score_at_index 单一真相源
